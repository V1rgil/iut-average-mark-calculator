...
async function main() {
    ...
    initLocalStorage().then(async () => {
        // Create the HTML structure of the extension.
        const extensionHtml = `
            <head>
                <meta charset="UTF-8">
                <meta name="viewport" content="width=device-width, initial-scale=1.0">
                <link href="${chrome.runtime.getURL('bootstrap/bootstrap.min.css')}" rel="stylesheet">
                <title>Notes</title>
            </head>
        ...
        // Parse the extension HTML string to a page
        const parser = new DOMParser();
        const doc = parser.parseFromString(extensionHtml, 'text/html');

        // Replace the entire page with the extension's page
